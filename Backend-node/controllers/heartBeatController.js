...
export const saveHeartBeat = asyncHandler(async (req, res) => {
  try {
    ...
    const oldTime = oldTimestamp ? new Date(oldTimestamp) : null;
    const newTime = newTimestamp ? new Date(newTimestamp) : null;

    if (
      (oldTime && isNaN(oldTime.getTime())) ||
      (newTime && isNaN(newTime.getTime()))
    ) {
    ...
    if (!tenantDb) {
      return res
        .status(500)
        .json({ error: "Failed to connect to tenant database." });
    }

    // ✅ Ensure HeartBeat model is registered correctly
    const HeartBeat =
      tenantDb.models.HeartBeat || tenantDb.model("HeartBeat", HeartBeatSchema);
...
export const fetchHeartBeat = asyncHandler(async (req, res) => {
  try {
    ...
    if (
      heartBeatData.timestamp < cutoffDate &&
