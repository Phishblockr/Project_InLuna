...
export const AuthProvider = ({ children }) => {
    ...
    const login = async (loginData) => {
        try {
            ...
            const userTypeAdmin = import.meta.env.VITE_USERTYPE_ADMIN;

            if (decodedToken.userType === userTypeAdmin) {
                setIsAuthenticated(true);
                scheduleTokenRefresh(data.token);
                setAccessToken(data.token)
                toast.success("Admin successful");
                window.location.href = import.meta.env.VITE_LOGIN_URL
            } else if (decodedToken.userType == userType) {
                setIsAuthenticated(true);
                scheduleTokenRefresh(data.token);
                setAccessToken(data.token)
    ...
    const clearAuthState = (clearOrgId) => {
        ...
        // navigate(`${import.meta.env.LOGIN_URL}/login`);
        window.location.href = import.meta.env.VITE_LOGIN_URL
    };
