...
const OrgLayout = () => {
  ...
  useEffect(() => {
    ...
    const fetchOrgDetails = async ({ orgId, token }) => {
      ...
      try {
        ...
        toast.error("Error fetching the Organisation details:", error.message);
      }
    };
  ...
  const filteredUsers = users
    ...
    .filter((user) =>
      selectedUserType !== "all" ? user.userType === selectedUserType : true
    ); // Fix here
        ...
        <div className="z-1 flex flex-col justify-between relative right-0 bottom-0 p-4 gap-4">
          <div>
            <div className="flex items-center gap-x-5 mt-5 bg-white py-3 px-2 rounded-xl shadow-sm">
              ...
              <select
                className="rounded-lg border-gray-300 border-2 p-2"
              ...
              >
                ...
                {users.length > 0 && (
                  <option
                    value={
                      users.find((user) => user.userType !== Admin)?.userType ||
                      "User"
                    }
                  >
                    User
          ...
          <div className="z-1 w-full bg-white rounded-xl shadow-sm p-3 h-max dark:bg-[#002451] dark:text-[#F4F4F4] dark:shadow-none">
            <nav className="flex gap-x-1 justify-between">
              ...
              >
                <MdOutlineArrowBackIos className="w-6 h-6" /> Prev
              </button>
              <span>
