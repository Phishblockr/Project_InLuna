...
const EditModal = ({ isOpen, quizId, onClose, onUpdated }) => {
  ...
  // Load departments (same as CreateModal)
  useEffect(() => {
    if (!isOpen) return;
    const load = async () => {
      setDeptLoading(true);
  ...
  useEffect(() => {
    ...
    const loadQuiz = async () => {
      ...
      try {
        const token = getToken();
        const data = await getQuiz(quizId, token); // returns { success, quiz }
  ...
  const changeQuestionType = (qIdx, type) => {
    setForm((f) => {
      ...
      if (type === "true-false") {
        ...
        q.options = [
          { text: "True", media: "", correct: prev ? !!prev.correct : true },
          { text: "False", media: "", correct: prev ? !prev.correct : false },
        ];
