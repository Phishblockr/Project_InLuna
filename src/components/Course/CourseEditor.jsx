...
const CourseEditor = () => {
  ...
  // TipTap editor handled via RichTextEditor component
  ...
  const handleVideoUpload = async (index, file, courseName, lectureTitle) => {
    ...
    const videoName = `${sanitizedCourseName}-Lecture-${
      index + 1
    }-${sanitizedLectureTitle}${file.name.substring(
  ...
  const handleGetSignedUrl = async (fileName) => {
    try {
      const res = await fetch(
        `${apiUrl}/course/getSignedUrl?fileName=${encodeURIComponent(
          fileName
        )}`,
        {
          method: "GET",
    ...
    <div className="z-1 min-h-[calc(100vh-65px)] flex flex-col justify-between relative right-0 bottom-0 p-4 gap-4">
      ...
      <div>
        ...
        <div>
          ...
          {/* Lecture section */}
          <div>
            <h2 className="block text-lg font-medium mb-2 mt-5">
              Lecture Structure
            </h2>
              ...
              <div className="mb-5">
                <div className="flex flex-row gap-5 border-2 rounded bg-white p-2">
                  <div className=" justify-center items-center w-full mx-2 flex gap-2">
                    <label
                      className="text-lg font-medium w-[150px]"
                      htmlFor="lectureName"
                    >
                        ...
                        {video.url ? (
                          <div className="flex items-center gap-2">
                            <span
                              ...
                              onClick={() =>
                                handleGetSignedUrl(video.url.split("/").pop())
                              } // Extract file name
                            >
                              {video.url.split("/").pop()}
                            </span>
                            ...
                            <button
                              ...
                              onClick={() =>
                                handleDeleteVideo(index, video.url)
                              }
                            >
                              x
                            </button>
                    ...
                    <div>
                      ...
                      <div className="flex flex-col">
                        <label
                          ...
                          className="block text-lg font-medium mb-2"
                        >
                          Select Email Group for Email based Interactive
                          Lectures
                        </label>
                        <select
                          name="emailGroup"
                          ...
                          className="bg-white w-full p-2 border border-gray-300 rounded mb-4"
                          onChange={(e) =>
                            updateLecture(index, "assignEmail", e.target.value)
                          }
                        >
                        ...
                        <span className="text-gray-700">
                          If you don't want to include or send training email
                          after completion of video just leave field as it is.
                        </span>
                        <span className="text-gray-700">
                          When you select email groups emails will be randomly
                          assigned (with in given group) to the user after user
                          completes the video.
                        </span>
                      </div>
                    </div>
                    <button
                      className="my-2 text-red-700 bg-gray-200 border-2 border-gray-300 rounded w-[200px] mb-2"
      ...
      <span>
        Notice: If this course has already been assigned, saving it as a draft
        will temporarily restrict user access until it is published again.
      </span>
