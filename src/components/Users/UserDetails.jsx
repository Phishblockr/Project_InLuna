...
const UserDetails = () => {
  ...
  const [isHeartbeatSectionVisible, setIsHeartbeatSectionVisible] =
    useState(false);
    ...
    try {
      ...
      const mergedBarGraphData = allDatesInMonth.map((date) => {
        ...
        return {
          ...
          malwareHostedVisits:
            index !== -1 ? data.barGraphData.malwareHostedVisits[index] : 0,
          phishingVisits:
  ...
  const fetchHeartBeatStatus = async (userId) => {
    ...
    try {
      ...
      const response = await fetch(
        `${apiUrl}/heartBeat/fetchHeartBeat/${userId}`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const overviewPoints = [
    ...
    },
    {
      id: 2,
      title: "Url Requests",
      count: activityCounts.requests,
      lastMonth: activityCounts.percentageReq,
      logo: "bi bi-shield-exclamation",
    },
    {
      id: 3,
      title: "Visits to Requested Urls.",
      count: activityCounts.visitsToRequestedUrls,
      lastMonth: activityCounts.percentageVisitToRequestedUrls,
      ...
      <div className="z-1 w-full bg-white rounded-xl shadow-xl p-3 h-max dark:bg-[#002451] dark:text-[#F4F4F4] dark:shadow-none">
        ...
        <div
          className={`rounded-lg shadow border-2 border-gray-100 dark:bg-[#001C40] dark:shadow-none dark:border-[#001C40] w-full p-5 overflow-hidden transition-all duration-500 ease-in-out ${
            isHeartbeatSectionVisible
              ? " mt-5 max-h-screen opacity-100"
              : "max-h-0 opacity-0"
          }`}
        ...
        >
          ...
          ) : heartbeatStatus.downtime &&
            heartbeatStatus.downtime.length > 0 ? ( // Render table if downtime data exists
        ...
        >
          <div className="flex flex-row gap-2 items-center  mb-5">
            ...
            <button
              ...
              onClick={() =>
                navigate(`/training/individualTraining/${user._id}`)
              }
            >
              Assign New Course
            </button>
