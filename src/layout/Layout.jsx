...
const Layout = () => {
  ...
  const showSidebarAndNavbar =
    ...
    !isResetPasswordPage &&
    !isForgotDetailsPage &&
    location.pathname !== "/passwordResetSuccessful" &&
    location.pathname !== "/passwordSetSuccessful";
    ...
    <div className="min-h-screen dark:bg-[#001733]">
      ...
      <div className="flex">
        {showSidebarAndNavbar && (
          <div
            className={`${sidebarWidth} fixed left-0 top-0 h-full z-10 transition-all`}
          >
            <Sidebar2 expanded={expanded} setExpanded={setExpanded} />
          </div>
        )}
        <div
          className={`flex-grow transition-all ${showSidebarAndNavbar ? contentMargin : ""}`}
        >
          <Outlet />
        </div>
      </div>
