...
const Overview = () => {
    ...
    const OverviewCard = ({ title, metrics, href }) => {
        ...
        return (
            <Wrapper
                ...
                className={`border border-dashed border-gray-200 rounded-lg bg-white flex justify-center items-center py-6 px-2 shadow flex-col text-center dark:text-[#F4F4F4] dark:bg-[#002451] dark:border-0 ${
    ...
    const fetchWeeklySignUp = async (currentMetrics) => {
        try {
            const res = await fetch(
                `${apiUrl}/supermetrics/getLastWeekSignups`,
                {
                    method: "GET",
                },
            );
    ...
    const fetchMonthSignUp = async (currentMetrics) => {
        try {
            const res = await fetch(
                `${apiUrl}/supermetrics/getLastMonthSignups`,
                {
                    method: "GET",
                },
            );
    ...
    const fetchLastFiveWeekSignup = async () => {
        try {
            const res = await fetch(
                `${apiUrl}/supermetrics/lastFiveWeekSignups`,
                {
                    method: "GET",
                },
            );
                ...
                .map((entry, i) => {
                    ...
                    return {
                        date: new Date(week.endDate)
                            .toISOString()
                            .split("T")[0], // formatted date string
    ...
    return (
        <div className="overflow-x-hidden">
            <div className="py-4 px-2 m-2 rounded-md">
                <OverviewCards data={metricsData} />
            </div>
            <div className="flex ">
                <div className="bg-white dark:bg-[#002451] dark:text-white py-4 px-5 m-2 mt-6 rounded-md mb-20 w-full">
                ...
                <div className="bg-white dark:bg-[#002451] dark:text-white py-4 px-3 m-2 mt-6 rounded-md mb-20 w-full">
                    <h2 className="text-xl font-semibold mb-8">
