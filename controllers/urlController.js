...
export const unshortenUrl = async (req, res) => {
  ...
  try {
    ...
    if (response.status === 301 || response.status === 302) {
      ...
      return res.json({
        requested_url: shortUrl,
        resolved_url: expandedUrl,
        success: true,
      });
      ...
      return res
        ...
        .json({ error: "Unable to resolve URL", status: response.status });
    }
  } catch (error) {
    ...
    if (error.code === "EAI_AGAIN") {
      return res
        .status(503)
        .json({ error: "DNS resolution failed. Please try again later." });
    }
    return res
      .status(500)
      .json({ error: "Failed to expand URL", details: error.message });
  }
};
...
export const deleteUrl = asyncHandler(async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: `URL ${urlData.url} deleted successfully`, id });
  } catch (error) {
...
export const addUrlFromCsv = asyncHandler(async (req, res) => {
  ...
  const { urlHeader, categoryHeader, status, isPhishing, isVerified } =
    req.body;
  ...
  try {
      ...
      fs.createReadStream(filePath)
        ...
        .on("data", (row) => {
          ...
          const category = categoryHeader
            ? row[categoryHeader]?.split(",").map((tag) => tag.trim()) || []
            : [];
    ...
    if (urls.length > 0) {
      ...
      res
        .status(400)
        .json({ message: "No valid URLs to add or all URLs are duplicates" });
    }
  } catch (error) {
...
export const fetchUrl = asyncHandler(async (req, res) => {
  try {
    ...
    if (!urlData) {
      return res
        .status(404)
        .json({ error: "URL does not exist in the database." });
    }
