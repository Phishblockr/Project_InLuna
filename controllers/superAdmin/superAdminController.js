  ...
  try {
    ...
    const logsWithDetails = await Promise.all(
      logs.map(async (log) => ({
        ...
        entityDetails:
          log.entityDetails ||
          (await getEntityDetails(log.entityType, log.entityId)),
      })),
    );
  ...
  try {
    ...
    const fields = [
      ...
      { label: "Entity Type", value: "entityType" },
      {
        ...
        value: (row) => JSON.stringify(row.entityDetails) || "NA",
      },
    ...
    res.download(filePath, `superadmin_logs_${Date.now()}.csv`, (err) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({ message: "Error downloading the CSV file" });
      }
    ...
    res
      .status(500)
      .json({ message: "Error exporting logs to CSV", error: error.message });
  }
});
...
const getDateRange = (filter) => {
  ...
  switch (filter) {
    ...
    case "last_week":
      return {
        createdAt: {
          ...
          $lte: endOfWeek(subWeeks(today, 1)),
        },
      };
    case "this_month":
      return {
    ...
    case "this_quarter":
      return {
        createdAt: { $gte: startOfQuarter(today), $lte: endOfQuarter(today) },
      };
    case "last_quarter":
      return {
        createdAt: {
          $gte: startOfQuarter(subQuarters(today, 1)),
          $lte: endOfQuarter(subQuarters(today, 1)),
        },
      };
    case "this_year":
      return {
        createdAt: { $gte: startOfYear(today), $lte: endOfYear(today) },
      };
    case "last_year":
      return {
        createdAt: {
          $gte: startOfYear(subYears(today, 1)),
          $lte: endOfYear(subYears(today, 1)),
        },
      };
    default:
