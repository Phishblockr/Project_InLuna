...
export const addRequestExt = asyncHandler(async (req, res) => {
  try {
    ...
    let existingReputation = await DomainReputation.findOne({
      urlDomain: domain,
    });
    ...
    if (!existingUrl || reqOption === "blacklist") {
    ...
    } else if (!existingUrl || reqOption === "whitelist") {
...
export const deleteRequest = asyncHandler(async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Request deleted successfully", requestId: id });
  } catch (error) {
